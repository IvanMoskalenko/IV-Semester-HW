...
let rec getValidPathToSave () =
    printf "%s" "Please, enter path: "
    let path = Console.ReadLine ()
    let directoryExists = path |> Path.GetDirectoryName |> Directory.Exists
    if directoryExists then path else
...
let rec program book =
    ...
    | "3" ->
        printfn $"%d{findPhone (getName ()) book}"
        program book
    | "4" ->
        printfn $"%s{findName (getPhone ()) book}"
        program book
    | "5" ->
        printBook book
        program book
    | "6" ->
        ...
        program book
    | "7" -> program (readBook (getValidPathToLoad ()))
    | _ ->
        printHelp ()
        program book
